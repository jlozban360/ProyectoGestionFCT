...
const VALID_ROL = ['ADMIN', 'COLABORADOR'];
...
// POST /api/profesores  (solo ADMIN)
router.post('/', requireAdmin, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [nombre, email, hashed, telefono ?? null, VALID_ROL.includes(rol) ? rol : 'COLABORADOR', activo]
    );
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    // Solo ADMIN puede editar a otros; cualquiera puede editarse a sí mismo
    ...
    const params = [
      ...
      rol && VALID_ROL.includes(rol) ? rol : existing.rows[0].rol,
...
router.delete('/:id', requireAdmin, async (req, res, next) => {
  try {
    const { rowCount } = await pool.query('DELETE FROM profesores WHERE id = $1', [req.params.id]);
    if (!rowCount) {
      return res.status(404).json({ status: 404, message: `Profesor no encontrado: ${req.params.id}` });
    }
    res.status(204).send();
